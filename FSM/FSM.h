...
inline void startBumpCount() { 
  st::previous_bumps = st::bumps; 
}
inline int  bumpCount()	{ 
  return st::bumps - st::previous_bumps; 
}

// Functions for timing
inline void startTimerDelay() { 
  st::timer_prev = millis(); 
}
inline unsigned long timerDelay() { 
  return millis() - st::timer_prev; 
}

// Functions for color counting
void startColorCount() { 
  st::previous_colorchange = st::colorchanges; 
}
int  colorCount() { 
  return st::colorchanges - st::previous_colorchange; 
}
...
namespace FSM
{
  ...
  bool waitFrontBump(int timeout = 0);

  // State Timers
  unsigned long previous_millis;
  inline long state_time() { 
    return millis() - previous_millis; 
  };
  inline long game_time() { 
    return millis(); 
  };
